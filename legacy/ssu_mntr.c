...
time_t getdtime(const char *fname)  //info에서 삭제된 시간을 가져온다
{
    FILE *ap;
    char abuf[PATH_MAX];
    struct tm at;
...
time_t getmtime(const char *fname)  //info에서 변경된 시간을 가져온다
{
    FILE *ap;
    char abuf[PATH_MAX];
    struct tm at;
...
int getdirsize(const char *dirpath) //디렉토리의 크기를 가져온다:내부파일들 크기의 합
{
    struct stat statbuf;
    struct dirent **items;
    ...
    for(i=0;i<itemnum;i++)
    {
        ...
        if(S_ISDIR(statbuf.st_mode))    //디렉토리인 경우
            size+=getdirsize(itempath); //그 하위디렉토리도 크기에 추가
        else
...
void do_size_d(const char *papath, const char *prpath, const char *fname,int depth) //해당 디렉토리의 크기, 하위디렉토리를 depth를 보고 출력
{
    struct stat stbuf;
    struct dirent **items;
...
int do_size(int pargc, char *pargv[ARGNUM])
{
    ...
    if(S_ISDIR(stbuf.st_mode) && dOption>1) //하위 디렉토리 출력
    {
        ...
        for(i=0;i<itemnum;i++)
        {
            sprintf(itempath,"%s/%s",oldpath,items[i]->d_name);
            lstat(itempath,&stbuf);
            if(S_ISDIR(stbuf.st_mode))  //디렉토리인 경우
                do_size_d(oldpath,newpath,items[i]->d_name,dOption-1);  //d옵션 깊이를 감안하고 디렉토리의 크기를 가져온다
            else    //파일의 경우 바로 출력하기
                printf("%ld\t\t%s/%s\n",stbuf.st_size,newpath,items[i]->d_name);
            free(items[i]);
        }
...
int do_recover(int pargc, char *pargv[ARGNUM])
{
    ...
    else if(c > 1)    //여러 개 있는 경우
    {
        ...
        for (i=0;i<itemnum;i++)
        {
            ...
            if(!strcmp(newpath,filename))
            {
                ...
                strftime(timememo,BUFSIZE,TIMEFORMAT,localtime(&dtime));
                printf("[%d] %s \tDeleted : %s\t",++c,newpath,timememo);    fmap[c]=i;  //선택지를 출력하고 그 파일을 fmap에 메모해둔다
    ...
    sprintf(oldpath,"%s/trash/files/%s",programpath,items[li]->d_name); //trash/files 경로
    getpath(items[li]->d_name,newpath);
    strncpy(npath,newpath,strlen(newpath)-strlen(filename)-1);          //경로에서 파일이름을 뺀 디렉토리까지 경로를 저장한다
    sprintf(infopath,"%s/trash/info/%s",programpath,items[li]->d_name); //trash/info 경로
